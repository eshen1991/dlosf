 ...
 *
 */
public class BotManager implements alphabetsoup.waypointgraph.BucketbotManager, Updateable, Renderable {
...
//													(map.getHeight()-2*r.getRadius()) * rand.nextFloat() + r.getRadius()) );
	...
	public void taskComplete(Bucketbot r, BucketbotTask t) {
		...
		for(Bucket b : SimulationWorldGraph.getSimulationWorld().getBuckets()) {
	...
	public void render() {
		...
		for(Bucket b : usedBucketStorageLocations.keySet()) {
			...
			disk.draw(0.0f, SimulationWorldGraph.getSimulationWorld().map.getTolerance(), 10, 1);
			GL11.glPopMatrix();
