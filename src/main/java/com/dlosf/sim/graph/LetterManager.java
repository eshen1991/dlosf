...
public class LetterManager implements Updateable {
	...
	public void update(double last_time, double cur_time) {
		for(LetterStation s : SimulationWorldGraph.getSimulationWorld().getLetterStations()) {
			...
			if(s.getAssignedLetters().size() < s.getCapacity()) {
				...
				//s.addLetter(SimulationWorldGraph.simulationWorld.wordList.generateRandomLetter());
				...
				if(requiredLetters.size() > 0) {
					...
					s.addBundle(l);
					SimulationWorldGraph.getSimulationWorld().bucketbotManager.newLetterBundleAssignedToStation(l, s);
				}
			}
		}
	}
