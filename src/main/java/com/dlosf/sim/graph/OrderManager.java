...
public class OrderManager implements Updateable {
	...
	public void update(double last_time, double cur_time) {
		WordList wl = SimulationWorldGraph.getSimulationWorld().getWordList();
		LetterManager lm = ((LetterManager)((SimulationWorldGraph) SimulationWorldGraph.getSimulationWorld()).letterManager);
		
		//see if all done
		if(wl.getAvailableWords().size() == 0)
			return;
		
		for(WordStation s : SimulationWorldGraph.getSimulationWorld().getWordStations()) {
			//give the station words if it needs them
			if(s.getAssignedWords().size() < s.getCapacity()) {
				Word w = wl.takeAvailableWord(0);
				s.assignWord(w);
				SimulationWorldGraph.getSimulationWorld().bucketbotManager.newWordAssignedToStation(w, s);
