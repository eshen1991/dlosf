...
import com.dlosf.sim.greedy.BotGlobalResource.LetterStationPickupRequest;
import com.dlosf.sim.greedy.BotGlobalResource.WordStationDeliveryRequest;
import alphabetsoup.waypointgraph.BucketbotDriver;
import alphabetsoup.waypointgraph.BucketbotManager;
 ...
 * @author Chris Hazard
 */
public class BotManager implements BucketbotManager, Updateable {
	
	BucketbotDriver bucketbot = null;
	BotGlobalResource manager = null;
	Map map = null;
	...
	public BotManager(BucketbotDriver bucketbot) {
	...
	public Bucket getBestDeliveryTask() {
		...
		if(bucketbot.getBucket() != null)
			base_time += bucketbot.getBucketPickupSetdownTime();
	...
	public void cancelCurrentLetterDelivery() {
		if(targetLetter != null && targetWord != null && targetWordStation != null)
			manager.openLetterRequests.add(new BotGlobalResource.WordStationDeliveryRequest(targetLetter, targetWord, targetWordStation));
		targetLetter = null;
		targetWord = null;
	...
	public void cancelCurrentLetterPickup() {
		if(targetLetter != null && targetLetterStation != null)
			manager.availableLetters.add(new BotGlobalResource.LetterStationPickupRequest(targetLetter, targetLetterStation));
		targetLetter = null;
	...
	/**Bucketbots should call requestNewTask of their corresponding BotManager when they are idle and have no tasks
	 */
	public void requestNewTask(Bucketbot r) {
		manager = SimulationWorldGreedy.getSimulationWorld().bucketbotGlobalResource;
		map = SimulationWorldGreedy.getSimulationWorld().map;
		waypointGraph = SimulationWorldGreedy.getSimulationWorld().waypointGraph;
	...
	/**Bucketbots should call their corresponding BotManager's taskComplete when an assigned task has been completed
	...
	/**Bucketbots should call their corresponding BotManager's taskAborted when an assigned task has been aborted
	 * @param r bucketbot calling the function 
