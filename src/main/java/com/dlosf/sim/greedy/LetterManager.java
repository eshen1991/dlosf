...
public class LetterManager implements Updateable {
	...
	public void update(double last_time, double cur_time) {
		for(LetterStation s : SimulationWorldGreedy.getSimulationWorld().getLetterStations()) {
			...
			if(s.getAssignedLetters().size() < s.getCapacity()) {
				...
				if(requiredLetters.size() > 0) {
					...
					s.addBundle(l);
					SimulationWorldGreedy.getSimulationWorld().bucketbotGlobalResource.newLetterBundleAssignedToStation(l, s);
				}
			}
		}
	}
