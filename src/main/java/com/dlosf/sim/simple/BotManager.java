...
public class BotManager implements Updateable {
	...
	public void newWordAssignedToStation(Word w, WordStation s) {
		MersenneTwisterFast rand = SimulationWorldSimple.rand;
	...
	public void requestNewTask(Bucketbot r) {
		...
		alphabetsoup.framework.Map map = SimulationWorldSimple.getSimulationWorld().map;
		MersenneTwisterFast rand = SimulationWorldSimple.rand;
	...
	public void taskComplete(Bucketbot r, BucketbotTask t) {
		...
		for(Bucket b : SimulationWorldSimple.getSimulationWorld().getBuckets()) {
