...
public class Pod extends BucketbotBase {
	...
	public <T> void assignTask(T tt) {
		...
		case STORE_BUCKET:
			if(t.getBucket() != getBucket()) {
				SimulationWorldSimple.getSimulationWorld().bucketbotManager.taskAborted(this, t);
				return;
			}
	 ...
	 */
	public void idle() {
		SimulationWorldSimple.getSimulationWorld().bucketbotManager.taskComplete(this, getCurrentTask());
		SimulationWorldSimple.getSimulationWorld().bucketbotManager.requestNewTask(this);
	...
	public class BucketbotPickupBucket implements BucketbotState {
		...
		public void act(BucketbotBase self) {
			...
			if(pickupBucket(bucket)) {
				...
				SimulationWorldSimple.getSimulationWorld().bucketbotManager.taskAborted(self, getCurrentTask());
				stateQueue.clear();
			}
		}
	}
	...
	public class BucketbotGetLetter implements BucketbotState {
		...
		public void act(BucketbotBase self) {
			...
			if(self.getDistance(station.getX(), station.getY()) > SimulationWorldSimple.getSimulationWorld().getMap().getTolerance()) {
	...
	public float getBestEvadeDirection(float visible_distance) {
		alphabetsoup.framework.Map map = SimulationWorldSimple.getSimulationWorld().getMap();
	...
	public class BucketbotEvade implements BucketbotState {
		...
		public void act(BucketbotBase self) {
			...
			if(stateQueue.size() > 1
					...
					&& !SimulationWorldSimple.getSimulationWorld().map.isBucketbotMoveValid(self,
	...
	public class BucketbotMove implements BucketbotState {
		...
		public void act(BucketbotBase self) {
			Map map = SimulationWorldSimple.getSimulationWorld().getMap();
