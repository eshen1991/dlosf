...
import com.dlosf.sim.simple.InveotryManager;
import com.dlosf.sim.simple.OrderManager;
import com.dlosf.sim.simple.Pod;
 ...
 * @author Chris Hazard
 */
public class SimulationWorldStaticInitializer extends SimulationWorld {
	...
	private static SimulationWorldStaticInitializer simulationWorldExample;
	public static SimulationWorldStaticInitializer getSimulationWorld() {
		return simulationWorldExample;
	}

	public SimulationWorldStaticInitializer() {
	...
	public static void main(String[] args) {
		simulationWorld = new SimulationWorldStaticInitializer();
		Map map = simulationWorld.getMap();
		...
		try {
			List<Bucketbot> bots = map.getBucketbots();
			for (Bucketbot bot : bots) {
				mapper.writerWithDefaultPrettyPrinter().writeValue(new File("bots.json"), bot);
				//mapper.writeValue(new File("bots.json"), bot);
			}
...
/*		if(simulationWorld.isUsingGUI())
		{
			RenderWindow.mainLoop(simulationWorld,
					((SimulationWorldStaticInitializer)simulationWorld).simulationDuration);
			RenderWindow.destroyUserInterface();
		}
		else
			simulationWorld.update( ((SimulationWorldStaticInitializer)simulationWorld).simulationDuration);
